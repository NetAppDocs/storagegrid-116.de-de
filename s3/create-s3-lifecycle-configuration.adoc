...
Aus diesem Grund kann ein Objekt aus dem Grid entfernt werden, obwohl die Speicheranweisungen in einer ILM-Regel noch auf das Objekt gelten. Alternativ kann ein Objekt auch dann im Grid aufbewahrt werden, wenn eine ILM-Platzierungsanleitung für das Objekt abgelaufen ist. Weitere Informationen finden Sie unter xref:../ilm/how-ilm-operates-throughout-objects-life.adoc[Funktionsweise von ILM während der gesamten Nutzungsdauer eines Objekts].
...
NOTE: Da der Bucket-Lebenszyklus ILM überschreibt, wird der `expiry-date` Hier wird das tatsächliche Datum angezeigt, an dem das Objekt gelöscht wird. Weitere Informationen finden Sie unter xref:../ilm/how-object-retention-is-determined.adoc[Wie die Aufbewahrung von Objekten bestimmt wird].
