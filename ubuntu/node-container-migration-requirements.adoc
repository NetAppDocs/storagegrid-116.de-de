...
Bevor Sie eine Migration oder eine Hypervisor-Wartung durchführen, müssen Sie die Nodes ordnungsgemäß herunterfahren. Siehe Anweisungen für xref:../maintain/shutting-down-grid-node.adoc[Herunterfahren eines Grid-Node].
...
Cold-Migration wird unterstützt. Bei der „Cold“-Migration sollten Sie die StorageGRID Nodes herunterfahren, bevor Sie sie zwischen Hosts migrieren. Siehe Anweisungen für xref:../maintain/shutting-down-grid-node.adoc[Herunterfahren eines Grid-Node].
...
Es gibt viele Möglichkeiten, eine konsistente Netzwerkschnittstelle über mehrere Hosts hinweg zu benennen; siehe xref:configuring-host-network.adoc[Konfigurieren Sie das Hostnetzwerk] Für einige Beispiele.
