...
Wiederherstellungspaket `.zip` Datei
 a| 
Unbedingt xref:downloading-recovery-package.adoc[Laden Sie das neueste Wiederherstellungspaket herunter] `.zip` Datei (`sgws-recovery-package-_id-revision_.zip`). Sie können die Recovery Package-Datei verwenden, um das System wiederherzustellen, wenn ein Fehler auftritt.
